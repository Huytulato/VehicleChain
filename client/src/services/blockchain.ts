 ...
 * Contract struct fields: vin, ipfsHash, plateNumber, brand, owner, pendingBuyer, status, rejectReason, timestamp
 */
...
export const getPendingRegistrations = async (): Promise<Vehicle[]> => {
  try {
    ...
    // Map contract struct to Vehicle type and filter for pending registration status
...
/**
 * Get all vehicles for authority dashboard (registrations + transfers)
 */
export const getAllVehiclesForAuthority = async (): Promise<Vehicle[]> => {
  try {
    console.log('Getting all vehicles for authority');
    const contract = await getContract();
    const allVehicles = await contract.getAllVehicles();
    return allVehicles.map(mapContractVehicle);
  } catch (error) {
    console.error('Error getting all vehicles for authority:', error);
    return [];
  }
};
...
/**
 * Get all vehicle activities (registrations + transfers) from events
 * This builds a complete history of all activities, not just current state
 */
export const getAllVehicleActivities = async (): Promise<VehicleActivity[]> => {
  try {
    console.log('Getting all vehicle activities from events');
    const contract = await getContractReadOnly();
    const provider = new ethers.BrowserProvider(window.ethereum);
    
    // Get all vehicles to supplement activity data
    const allVehicles = await contract.getAllVehicles();
    ...
    // Query all events
    const filterFrom = 0; // From block 0 (or use deployment block if known)
    const currentBlock = await provider.getBlockNumber();

    // 1. YeuCauMoi events (Registration requests)
    const registrationRequestEvents = await contract.queryFilter(
      contract.filters.YeuCauMoi(),
      filterFrom,
      currentBlock
    );
    
    for (const event of registrationRequestEvents) {
      const vin = event.args.vin;
      const owner = event.args.owner;
      const vehicle = vehicleMap.get(vin);
      const block = await provider.getBlock(event.blockNumber);
      
      // Check if this was approved or rejected
      let status: 'PENDING' | 'APPROVED' | 'REJECTED' = 'PENDING';
      let rejectReason = '';
      
      if (vehicle) {
        const vehicleStatus = mapContractStatus(Number(vehicle.status));
      ...
      activities.push({
        id: `reg_req_${vin}_${event.blockNumber}_${event.logIndex}`,
        vin,
        ...
        photoIpfsHash: vehicle?.ipfsHash || undefined,
      });
    }

    // 2. DaDuyetCapMoi events (Approved registrations)
    const registrationApprovedEvents = await contract.queryFilter(
      contract.filters.DaDuyetCapMoi(),
      filterFrom,
      currentBlock
    );
    
    for (const event of registrationApprovedEvents) {
      const vin = event.args.vin;
      const owner = event.args.owner;
      const vehicle = vehicleMap.get(vin);
      const block = await provider.getBlock(event.blockNumber);
      ...
      activities.push({
        id: `reg_approved_${vin}_${event.blockNumber}_${event.logIndex}`,
        vin,
        licensePlate: vehicle?.plateNumber || '',
        brand: vehicle?.brand || 'Unknown',
        activityType: 'REGISTRATION',
    ...
    // 3. YeuCauSangTen events (Transfer requests)
    const transferRequestEvents = await contract.queryFilter(
      contract.filters.YeuCauSangTen(),
      filterFrom,
      currentBlock
    );
    ...
    for (const event of transferRequestEvents) {
      ...
      if (vehicle) {
        const vehicleStatus = mapContractStatus(Number(vehicle.status));
        if (vehicleStatus === 'ACTIVE' && vehicle.pendingBuyer === ethers.ZeroAddress) {
          // If status is ACTIVE and no pending buyer, transfer was approved
          status = 'APPROVED';
        } else if (vehicleStatus === 'REJECTED') {
          status = 'REJECTED';
          rejectReason = vehicle.rejectReason || '';
        } else if (vehicleStatus === 'TRANSFERRING') {
          status = 'PENDING';
        }
      }

      activities.push({
        id: `transfer_req_${vin}_${event.blockNumber}_${event.logIndex}`,
        vin,
        licensePlate: vehicle?.plateNumber || '',
        brand: vehicle?.brand || 'Unknown',
    ...
    // 4. DaDuyetSangTen events (Approved transfers)
    const transferApprovedEvents = await contract.queryFilter(
      contract.filters.DaDuyetSangTen(),
      filterFrom,
      currentBlock
    );
    ...
    for (const event of transferApprovedEvents) {
      ...
      activities.push({
        id: `transfer_approved_${vin}_${event.blockNumber}_${event.logIndex}`,
        vin,
        licensePlate: vehicle?.plateNumber || '',
        brand: vehicle?.brand || 'Unknown',
        activityType: 'TRANSFER',
    ...
    // 5. Handle rejected vehicles (those with REJECTED status but no corresponding event)
    // We'll create activities for rejected vehicles that don't have a corresponding approved/rejected activity
    for (const vehicle of allVehicles) {
      const vehicleStatus = mapContractStatus(Number(vehicle.status));
      if (vehicleStatus === 'REJECTED' && vehicle.rejectReason) {
        const vin = vehicle.vin;
        // Check if we already have a rejected activity for this VIN
        const hasRejectedActivity = activities.some(
          a => a.vin === vin && a.status === 'REJECTED' && a.activityType === 'REGISTRATION'
        );
        
        if (!hasRejectedActivity) {
          // Create a rejected registration activity
          activities.push({
            id: `reg_rejected_${vin}_${vehicle.timestamp}`,
            vin,
            licensePlate: vehicle.plateNumber,
            brand: vehicle.brand || 'Unknown',
            activityType: 'REGISTRATION',
            status: 'REJECTED',
            fromAddress: vehicle.owner,
            timestamp: Number(vehicle.timestamp),
            rejectReason: vehicle.rejectReason || 'Không đạt yêu cầu',
            photoIpfsHash: vehicle.ipfsHash || undefined,
          });
        }
      }
